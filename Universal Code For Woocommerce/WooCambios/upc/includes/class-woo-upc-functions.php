...
if( !class_exists( 'Woo_UPC_Functions' ) ) {

    /**
     * Woo_UPC_Functions class
     *
     * @since       0.2.0
     */
    class Woo_UPC_Functions {

        /**
         * @var         Woo_UPC_Functions $instance The one true Woo_UPC_Functions
         * @since       0.2.0
         */
         ...
         * @access      public
         * @since       0.2.0
         * @return      object self::$instance The one true Woo_UPC_Functions
         */
        public static function instance() {
            if( !self::$instance ) {
                self::$instance = new Woo_UPC_Functions();
                self::$instance->hooks();
            }
        ...
        private function hooks() {
            ...
            add_filter( 'woocommerce_structured_data_product', array( $this, 'add_upc_to_structured_data' ), 10, 2 );
        ...
        public function scripts_styles( $hook ) {
            ...
            wp_enqueue_script( 'woo-add-upc', Woo_UPC_URL . 'assets/js/woo-add-upc' . $suffix . '.js', array( 'wc-add-to-cart-variation', 'jquery' ), Woo_UPC_VER, true );

            $localized = array(
                "upc" => get_post_meta( $post->ID, 'hwp_product_upc', 1 )
            );
            ...
            if( !empty( $variations ) ) {
                ...
                foreach ( $variations as $variation ) {
                    if( !empty( $variation ) && $variation['variation_is_active'] != false ) {
                        ...
                        $localized["variation_upcs"][$variation['variation_id']] = get_post_meta( $variation['variation_id'], 'hwp_var_upc', 1 );
                ...
                foreach ( $composite_variations as $id => $comp_variation ) {
                    ...
                    foreach ( $comp_variation as $variation ) {
                        if( !empty( $variation ) && $variation['variation_is_active'] != false ) {
                            ...
                            $localized["composite_variation_upcs"][$variation['variation_id']] = get_post_meta( $variation['variation_id'], 'hwp_var_upc', 1 );
            ...
            wp_localize_script( 'woo-add-upc', 'wooUpcVars', $localized );
        ...
        public function maybe_display_tn() {
            ...
            global $post;
            $upc = get_post_meta( $post->ID, 'hwp_product_upc', 1 );
            $display = get_option( 'hwp_display_upc' );
            $label = ( !empty( get_option( 'hwp_upc_text' ) ) ? get_option( 'hwp_upc_text' ) : 'UPC' );
            ...
            if( !empty( $upc ) ) {
                ...
                echo '<span class="hwp-upc">' . esc_html__( $label . ': ', 'woo-add-upc' ) . '<span>' . get_post_meta( $post->ID, 'hwp_product_upc', 1 ) . '</span></span>';
        ...
        /**
         * Adds UPC field to product JSON-LD.
         *
         * @since       0.5.0
         * @return      array    Modified WooCommerce product structured data.
         */
        public function add_upc_to_structured_data( $markup, $product ) {
		
            // Bail if product variable not available
            if ( ! $product ) { return $markup; }
            
            $product_id = $product->get_id();
            $upc = get_post_meta( $product_id, 'hwp_product_upc', 1 );
            $markup['upc'] = trim($upc);
            return $markup;
    ...
    $Woo_UPC_Functions = new Woo_UPC_Functions();
    $Woo_UPC_Functions->instance();
